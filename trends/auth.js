  ...
  if (r.data.session) {
    ...
    if (el) {
      el.innerHTML = getGreeting() + ', ' + name +
        ' &nbsp; <a href="#" onclick="signOut()" style="color:gray; border: 1px solid white; border-radius:7px;padding 3px;font-size:clamp(25px,30px,35px);">sign out</a>';
    }
    ...
    .then(function(data) {
      ...
      if (el) {
        el.innerHTML = getGreeting() + ', ' + name + badge +
                ' &nbsp; <a href="#" onclick="signOut()" style="color: white; border: 1px solid white; border-radius:7px;padding :3px;font-size:clamp(25px,30,35px);">sign out</a>'
      }
    })
    .catch(function() {});
    ...
    var el = document.getElementById('user-greeting');
    if (el) {
      el.innerHTML = '<a href="login.html" style="color:white;font-size: clamp( 20px, 25px,30px);  text-decoration:none;   border: 1px solid white;letter-spacing:0.1em; font-weight:200; border-radius: 15px; padding:3px;">Sign in</a>';
    }
  }
});
